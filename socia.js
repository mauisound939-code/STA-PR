...
(function sociaHelper() {
  ...
  function ensureRoot() {
    ...
    root.id = ROOT_ID;
    root.innerHTML =
      '<div id="' + LAUNCHER_WRAP_ID + '">' +
        '<img id="' + LAUNCHER_MASCOT_ID + '" src="https://i.postimg.cc/sxSKLppQ/LOGOTIPO-Y-MASCOTA-(3).png" alt="SOCIA">' +
        '<button id="' + LAUNCHER_ID + '" type="button">' +
          '<span class="socia-launcher-title">SOCIA te ayuda a surtir</span>' +
          '<span class="socia-launcher-sub">Arma tu pedido en segundos</span>' +
        '</button>' +
      '</div>' +
      '<div id="' + MODAL_ID + '">' +
  ...
  var launcherWrap = document.getElementById(LAUNCHER_WRAP_ID);
  if (!modal) return;
  ...
  function bindEvents() {
    document.addEventListener('click', function (event) {
      ...
      var launcher = target.closest ? target.closest('#' + LAUNCHER_ID) : null;
      if (launcher) {
        toggleModal(true);
        return;
      }
